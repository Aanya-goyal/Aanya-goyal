//Q119: Write a program to take an integer array as input. Only one element will be repeated. Print the repeated element.
// Try to find the result in one single iteration.

/*
Sample Test Cases:
Input 1:
nums1 = [1,3,3,4]
Output 1:
3

Input 2:
nums1 = [1,2,2]
Output 2:
2

Input 3:
nums1 = [0,4,1,1,5]
Output 3:
1

*/
#include <stdio.h>
#include <stdlib.h>
#define MAX_SIZE 1000
int findRepeatedNumber(int* nums, int size) {
    int seen[MAX_SIZE] = {0}; // Array to keep track of seen numbers
    for (int i = 0; i < size; i++) {
        if (seen[nums[i]] == 1) {
            return nums[i]; // Return the repeated number
        }
        seen[nums[i]] = 1; // Mark this number as seen
    }
    return -1; // In case no repeated number is found (should not happen as per problem statement)
}
int main() {
    int nums[MAX_SIZE];
    int size;
    
    // Read the size of the array
    printf("Enter the number of elements in the array: ");
    scanf("%d", &size);
    
    // Read the elements of the array
    printf("Enter the elements of the array (one element will be repeated): ");
    for (int i = 0; i < size; i++) {
        scanf("%d", &nums[i]);
    }
    
    // Find and print the repeated number
    int repeatedNumber = findRepeatedNumber(nums, size);
    printf("The repeated number is: %d\n", repeatedNumber);
    
    return 0;
}
